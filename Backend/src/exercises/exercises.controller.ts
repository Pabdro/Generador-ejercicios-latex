...
export class ExercisesController {
    ...
    //@UseGuards(JwtAuthGuard)
    @Post('/sentLatex')
    async sentLatex(@Res() res: Response, @Body() exerciseListFindDTO: ExerciseListFindDTO) {
    const exercises = [];
    console.log(exerciseListFindDTO);
    for (let i = 0; i < exerciseListFindDTO.ids.length; i++) {
        const exercise = await this.exercisesService.getExerciseById(exerciseListFindDTO.ids[i]);
        exercises.push(exercise);
    }
    ...
    const filePath = join(process.cwd(), 'src/exercises/src/the_latex_file_to_send/latex_folder/exercices.tex');
    res.download(filePath, 'exercises.tex');
    }
